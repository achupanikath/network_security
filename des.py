...
class des():
    ...
    def substitute(self, d_e):  # Substitute bytes using SBOX
        ...
        result = list()
        for i in range(0, 8):
            bin_value = self.compute_s_box(subblocks[i], i)
            result += [int(b) for b in bin_value]
        return result
    ...
    def compute_s_box(self, block, round):
        row = int(str(block[0])+str(block[5]), 2)  # grabs the row with the first and last bit
        col = int(''.join([str(x) for x in block[1:][:-1]]), 2)
        value = S_BOX[round][row][col]
        binl = binvalue(value, 4)
        return binl
